...
--    utf8mb4 is explicit here (not just inherited from the DB
--    default) because several of the seed symbols below (₹ ₺ ₩ €)
--    are multi-byte and will fail or get mangled under latin1 or
--    plain utf8 (MySQL/MariaDB's 3-byte "utf8" alias, not full
--    UTF-8).
...
ALTER TABLE `Expense` ADD COLUMN `currencyCode` VARCHAR(5) AFTER `currency`;
...
-- 5. Enforce NOT NULL + set the default.
ALTER TABLE `Expense`
    MODIFY COLUMN `currencyCode` VARCHAR(5) NOT NULL DEFAULT 'EUR';

-- 6. Drop the old column and its index, add the new index + FK.
DROP INDEX IF EXISTS `Expense_currency_idx` ON `Expense`;
